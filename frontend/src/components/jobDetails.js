...
const JobDetails = () => {
  ...
  const handleRatingSubmit = async () => {
    try {
      ...
      // Refetch reviews
      const reviewRes = await fetch(`http://localhost:3000/api/reviews/company/${job.company_id}`);
      const reviewData = await reviewRes.json();
      const avg = reviewData.reduce((sum, r) => sum + r.rating, 0) / reviewData.length;
      setAverageRating(avg.toFixed(1));
  ...
  return (
    ...
    <div style={styles.container}>
      ...
      <img src={`http://localhost:3000/jobimages/${job.image}`} alt={job.title} style={styles.jobImage} />
