...
const SavedJobs = () => {
  ...
  const fetchSavedJobs = async () => {
    try {
      const response = await fetch("https://four71project.onrender.com/api/saved_jobs/user/2");
      const data = await response.json();
      if (Array.isArray(data)) {
        const jobDetails = await Promise.all(
          data.map(async (job) => {
            const res = await fetch(`https://four71project.onrender.com/api/jobs/${job.job_id}`);
